...
int main(int argc, char* argv[]){
		...
		int el;
		double DPl, alpha_g, delta0l, q_star, step;
		VectorXd fl0_all, fl1_all_filtered;
...
//		std::cout << "      - Data file: " << filename_data << std::endl;
		...
		*/
		...
		std::cout << "  3. Computing " << el << " frequencies with the solver_mm.cpp::solve_mm_asymptotic_O2from_l0() function..." << std::endl;
		freqs_l1=solve_mm_asymptotic_O2from_l0(fl0_all, el, delta0l, DPl, alpha_g, q_star, sigma_p_l1, step, true, true);

		posOK=where_in_range(freqs_l1.nu_m, fl0_all.minCoeff(), fl0_all.maxCoeff(), false);
    	fl1_all_filtered.resize(posOK.size());
    	if (posOK[0] != -1){
        	for (int i=0; i<posOK.size();i++)
        	{
        	    fl1_all_filtered[i]=freqs_l1.nu_m[posOK[i]];
        	    if (sigma_m_l1 !=0) // If requested, we add a random gaussian qty to the mixed mode solution
        	    {
        	        ...
        	        fl1_all_filtered[i]=fl1_all_filtered[i]+r;
        	    }
        	}
    	}
		// Generating widths profiles for l=1 modes using the ksi function
    	VectorXd ksi_pg, ksi_pg_filtered, h1_h0_ratio, h1_h0_ratio_filtered;
    	...
    	ksi_pg_filtered=ksi_fct2(fl1_all_filtered, freqs_l1.nu_p, freqs_l1.nu_g, freqs_l1.dnup, freqs_l1.dPg, q_star, "precise"); //"precise" // assume Dnu_p, DPl and q constant
    	...
    	h1_h0_ratio_filtered=h_l_rgb(ksi_pg_filtered); // WARNING: Valid assummption only not too evolved RGB stars (below the bump, see Kevin mail 10 August 2019)
    
		std::cout << "  4. Writing Results on-screen... " << std::endl;
		...
		std::cout << "     - Mixed modes After Filtering by max(fl0_all) and max(fl0_all):" << std::endl;
		std::cout << fl1_all_filtered.transpose() << std::endl;
		std::cout << "     - ksi_pg with original values After Filtering by max(fl0_all) and max(fl0_all):" << std::endl;
		...
		std::cout << ksi_pg_filtered.transpose() << std::endl;
		...
		std::cout << "     - h1_h0_filtered with original values After Filtering by max(fl0_all) and max(fl0_all):" << std::endl;
		std::cout << h1_h0_ratio_filtered.transpose() << std::endl;
