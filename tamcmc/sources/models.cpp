...
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------
//                  ----- MODELS FOR EVOLVED STARS -----
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------

VectorXd model_Evolved_Global_a1etaa3_l1mixed(VectorXd params, VectorXi params_length, VectorXd x){
	/* Model of the power spectrum of a Main sequence solar-like star
	 * param is a vector of parameters
	 * param_length defines the structure of the parameters
	 * x is the frequency assumed to be in microHz
	...
	const int Nmax023=params_length[0]; // Number of Heights for l=0,2,3
	...
	const int Nsplit023=params_length[6]; // number of splitting/asym parameters for l=0,2,3. Should be 6 for a ALL global MS model (a1,eta,a3, magb, magalfa,asym)
	...
	VectorXd fl0_all(Nmax023), Wl0_all(Nmax023), noise_params(Nnoise); //Hl0_all[Nmax],
	double fl0, fl1, fl2, fl3, Vl1, Vl2, Vl3, Hl0, Hl1, Hl2, Hl3, Wl0, Wl1, Wl2, Wl3, a1,a11,eta,a3, asym;
	...
	/* --------- FAIL SAFES ------
	    - This routine is in beta ==> Not properly formlalised handling of IOs
            - Imposing some constraints to avoid user mishandling
	*/ 
	//std::cout << "params[Nmax023 + lmax + Nf + 1] = " << params[Nmax023 + lmax + Nf + 1] << std::endl;
	//std::cout << "params[Nmax023 + lmax + Nf + 2] = " << params[Nmax023 + lmax + Nf + 2] << std::endl;

	if (params[Nmax023 + lmax + Nf + 1] !=0 || params[Nmax023 + lmax + Nf + 2] !=0){
		std::cout << "   Using a beta version of 'model_Evolved_Global_a1etaa3_l1mixed'" << std::endl;
		std::cout << "   a3 and a2 not yet properly implemented " << std::endl;
		std::cout << "   Please set them to 0" << std::endl;
	...
	/*
	   ...
	   -------------------------------------------------------
	*/
	trunc_c=params[Nmax023+lmax+Nf+Nsplit023+Nwidth+Nnoise+Ninc];

	inclination=std::atan(params[Nmax023 + lmax + Nf+4]/params[Nmax023 + lmax + Nf+3]); 
	inclination=inclination*180./pi;
	a1=pow(params[Nmax023 + lmax + Nf+3],2)+ pow(params[Nmax023 + lmax + Nf+4],2);
	...
	if(lmax >=2){
		Vl2=std::abs(params[Nmax023+1]);
		ratios_l2=amplitude_ratio(2, inclination);
        	//std::cout << "Vl2 " << Vl2 << std::endl;
	}
	if(lmax >=3){
		Vl3=std::abs(params[Nmax023+2]);
		ratios_l3=amplitude_ratio(3, inclination);
	}

	fl0_all=params.segment(Nmax023 + lmax, Nfl0); // required for the interpolation of the widths
	Wl0_all=params.segment(Nmax023 + lmax + Nf + Nsplit023, Nmax023);

	eta=0;//params[Nmax + lmax + Nf + 1];
	a3=0;//params[Nmax + lmax + Nf + 2];
	asym=params[Nmax023+lmax + Nf + 5];
	...
	*/
	...
	for(long n=0; n<Nmax023; n++){		
		...
		//}
		if(lmax >=2){
			fl2=params[Nmax023+lmax+Nfl0+Nfl1+n];
			Hl2=Hl0*Vl2;
			Wl2=std::abs(lin_interpol(fl0_all, Wl0_all, fl2));
			model_final=optimum_lorentzian_calc_a1etaa3(x, model_final, Hl2, fl2, a1, eta, a3,asym, Wl2, 2, ratios_l2, step, trunc_c);
		}
        	if(lmax >=3){
			fl3=params[Nmax023+lmax+Nfl0+Nfl1+Nfl2+n];
			Hl3=Hl0*Vl3;
			Wl3=std::abs(lin_interpol(fl0_all, Wl0_all, fl3));
			model_final=optimum_lorentzian_calc_a1etaa3(x, model_final, Hl3, fl3, a1, eta, a3, asym, Wl3, 3, ratios_l3, step, trunc_c);
		}		
	...
	for(long n=0; n<Nfl1; n++){
			fl1=params[Nmax023+lmax+Nfl0+n];
			Hl1=std::abs(params[Nmax023+lmax+Nf+Nsplit023+Nwidth+Nnoise+Ninc+Ntrunc+n]);
			Wl1=std::abs(params[Nmax023+lmax+Nf+Nsplit023+Nwidth+Nnoise+Ninc+Ntrunc+NHl1+n]);
			a11=params[Nmax023+lmax+Nf+Nsplit023+Nwidth+Nnoise+Ninc+Ntrunc+NHl1+NWl1+n];
			model_final=optimum_lorentzian_calc_a1etaa3(x, model_final, Hl1, fl1, a11, eta, a3,asym, Wl1, 1, ratios_l1, step, trunc_c);
	...
	*/
	//std::cout << "Before computing noise model" << std::endl;
	noise_params=params.segment(Nmax023+lmax+Nf+Nsplit023+Nwidth, Nnoise);
	Nharvey=(Nnoise-1)/3;
	//std::cout << "Nharvey = " << Nharvey << std::endl;
