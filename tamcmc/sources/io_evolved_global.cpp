...
using Eigen::MatrixXd;
 ...
 // WARNING : JUST A COPY PASTE OF ..._MS_Global ... Merging will requires once getting into the release version
...
Input_Data build_init_evolved_Global(const MCMC_files inputs_MS_global, const bool verbose){
	...
	std::vector<double> f_inputs, h_inputs, w_inputs, f_priors_min, f_priors_max, f_el, h1_inputs, w1_inputs;
	std::vector<bool> f_relax, h_relax, w_relax, w1_relax, h1_relax; 
	std::vector<int> rf_el, rw_el, rh_el;
		
	Input_Data Snlm_in, Sn1m_in, Vis_in, Inc_in, Noise_in, freq_in, height_in, width_in; // This is by block, each category of parameters		
    ...
    for(int i=0; i<inputs_MS_global.common_names.size(); i++){
        if(inputs_MS_global.common_names[i] == "model_fullname" ){ // This defines if we assume S11=S22 or not (the executed model will be different)
            ...
            if(all_in.model_fullname == "model_Evolved_Global_a1etaa3_l1mixed"){
	...
	Vis_in.priors.setConstant(-9999); // Put a recognizable values to indicate empty slotsVis_in
	...
	for(int el=0; el<lmax+1; el++){
		...
		for(int en=0; en<pos_el.size(); en++){
			...
			if(el ==0){
				w_inputs.push_back(inputs_MS_global.eigen_params(pos_el[en],4)); // COLUMNS for el=0,2,3 FOR INTERPOLATED WIDTH USING l=0				
				h_inputs.push_back(inputs_MS_global.eigen_params(pos_el[en],5)); // COLUMNS for el=0,2,3 FOR SCALED HEIGHT USING VISBILITIES				
			}
			if(el ==1){
				w1_inputs.push_back(inputs_MS_global.eigen_params(pos_el[en],4)); // COLUMNS for widths of l=1 mixed modes
				h1_inputs.push_back(inputs_MS_global.eigen_params(pos_el[en],5)); // COLUMNS for heights of l=1 mixed modes
			}
	...
	// ------------------------------------------------------------------------------------------
   // Handling of the l=1 mixed modes 
   Sn1m_in.inputs_names.resize(Nf_el[1]);
   Sn1m_in.priors_names.resize(Nf_el[1]);
   Sn1m_in.priors.resize(Nmax_prior_params,Nf_el[1]);
   Sn1m_in.inputs.resize(Nf_el[1]); // contains a1, eta, a3, alfa (if any), beta (if any) and asym
   Sn1m_in.relax.resize(Nf_el[1]); // contains a1, eta, a3, alfa (if any), beta (if any) and asym
	...
	for(int i=0; i<inputs_MS_global.common_names.size(); i++){
		...
		if(inputs_MS_global.common_names[i] == "freq_smoothness" || inputs_MS_global.common_names[i] == "Freq_smoothness"){
			if(inputs_MS_global.common_names_priors[i] != "bool"){
				fatalerror_msg_io_evolved_Global("freq_smoothness", "bool", "[0/1]     [Smoothness coeficient in microHz]", "1     2.0" );
			} else{
		...
		if(inputs_MS_global.common_names[i] == "trunc_c"){
			if(inputs_MS_global.common_names_priors[i] != "Fix"){
				fatalerror_msg_io_evolved_Global("trunc_c", "Fix", "[Truncation parameter]", "20" );
			} else{
		...
		if(inputs_MS_global.common_names[i] == "splitting_a1" || inputs_MS_global.common_names[i] == "Splitting_a1"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("splitting_a1", "Fix_Auto", "", "" );
			}			
			...
			// --- Handling the splitting for mixed modes ---
			// Basically put all inputs/priors same as splitting_a1
			for(int kk=0; kk<Nf_el[1]; kk++){
				Sn1m_in.inputs_names[kk]="Splitting_a11"; 
				Sn1m_in.priors_names[kk]=inputs_MS_global.common_names_priors[i];
				Sn1m_in.inputs[kk]=inputs_MS_global.modes_common(i,0); // The input value is always in the first element.
				Sn1m_in.relax[kk]=Snlm_in.relax[0];
            			for(int k=0; k<Nmax_prior_params; k++){
            			    Sn1m_in.priors(k,kk)=Snlm_in.priors(k,0); // Copy the prior of average a1 on the individual a1(n)
            			}
			}
		}
		...
		if(inputs_MS_global.common_names[i] == "asphericity_eta"|| inputs_MS_global.common_names[i] == "Asphericity_eta"){  
			...
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){ // Case where the centrifugal force is added but FIXED.
				if(inputs_MS_global.modes_common(i,0) !=0 || inputs_MS_global.common_names_priors[i] != "Fix"){
					std::cout << "This is a beta version of io_evolved_global.cpp" << std::endl;
					std::cout << "Asphericity eta should be set to 0 with a 'Fix' prior for l=1 mixed modes" << std::endl;
					std::cout << "The program will exit now" << std::endl;
					exit(EXIT_SUCCESS);				
				}
		...
		if(inputs_MS_global.common_names[i] == "splitting_a3" || inputs_MS_global.common_names[i] == "Splitting_a3"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("splitting_a3", "Fix_Auto", "", "" );
			}
			...
			Snlm_in.inputs[2]=0;
			if(inputs_MS_global.modes_common(i,0) !=0){
				std::cout << "This is a beta version of io_evolved_global.cpp" << std::endl;
				std::cout << "a3 should be set to 0 for l=1 mixed modes" << std::endl;
				std::cout << "The program will exit now" << std::endl;
				exit(EXIT_SUCCESS);				
			}
			if(inputs_MS_global.common_names_priors[i] == "Fix"){
				...
				// -- Fail safe on a3 --
				std::cout << "This is a beta version of io_evolved_global.cpp" << std::endl;
				std::cout << "a3 should be set to 'Fix' for l=1 mixed modes" << std::endl;
				std::cout << "The program will exit now" << std::endl;
				exit(EXIT_SUCCESS);
		...
		if(inputs_MS_global.common_names[i] == "asymetry" || inputs_MS_global.common_names[i] == "Asymetry"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("asymetry", "Fix_Auto", "", "" );
			}
		...
		if(inputs_MS_global.common_names[i] == "visibility_l1" || inputs_MS_global.common_names[i] == "Visibility_l1"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("visibility_l1", "Fix_Auto", "", "" );
			}
			...
			if(lmax >= 1){
				...
				if(inputs_MS_global.common_names_priors[i] == "Fix"){
					...
					// -- Fail safe on l=1 vibilities --
					std::cout << "This is a beta version of io_evolved_global.cpp" << std::endl;
					std::cout << "Visibilities should be set to 'Fix' for l=1 mixed modes" << std::endl;
					std::cout << "The program will exit now" << std::endl;
					exit(EXIT_SUCCESS);
		...
		if(inputs_MS_global.common_names[i] == "visibility_l2" || inputs_MS_global.common_names[i] == "Visibility_l2"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("visibility_l2", "Fix_Auto", "", "" );
			}
		...
		if(inputs_MS_global.common_names[i] == "visibility_l3" || inputs_MS_global.common_names[i] == "Visibility_l3"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("visibility_l3", "Fix_Auto", "", "" );
			}
		...
		if(inputs_MS_global.common_names[i] == "inclination" || inputs_MS_global.common_names[i] == "Inclination"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("inclination", "Fix_Auto", "", "" );
			}
		...
		if(inputs_MS_global.common_names[i] == "sqrt(splitting_a1).cosi"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("sqrt(splitting_a1).cosi", "Fix_Auto", "", "" );
		...
		if(inputs_MS_global.common_names[i] == "sqrt(splitting_a1).sini"){
			if(inputs_MS_global.common_names_priors[i] == "Fix_Auto"){
				fatalerror_msg_io_evolved_Global("sqrt(splitting_a1).sini", "Fix_Auto", "", "" );
			}			
...
if((bool_a1cosi == 0) && (bool_a1sini == 0)){ // Case where Inclination and Splitting_a1 are supposed to be used 
	if( (all_in.model_fullname == "model_MS_Global_a1etaa3_HarveyLike") || (all_in.model_fullname == "model_MS_Global_a1etaa3_Harvey1985") || (all_in.model_fullname == "model_Evolved_Global_a1etaa3_l1mixed")){
...
if((bool_a1cosi == 1) && (bool_a1sini ==1)){
	...
	all_in.plength[11]=Nf_el[1]; // H(l=1);
	all_in.plength[12]=Nf_el[1]; // W(l=1);
	all_in.plength[13]=Nf_el[1]; // a11;
	...
	for(int i=0; i<all_in.plength[7]; i++){
		...
		if(w_relax[i] == 1){
			...
			if(inputs_MS_global.Dnu <= 40){ 
				all_in.priors(1, p0 + i)=inputs_MS_global.Dnu;  // Jeffreys prior, with hmax=40 or set to Dnu if Dnu is clearly for an evolved star... 
			} else{
				all_in.priors(1, p0 + i)=40.;
			}
	...
	// ------ l=1 MIXED MODES HANDLING --------
	// --- Put the Hl1 ---
	p0=all_in.plength[0] + all_in.plength[1] + all_in.plength[2] + all_in.plength[3] + all_in.plength[4] + all_in.plength[5] + all_in.plength[6] + all_in.plength[7] + all_in.plength[8] + all_in.plength[9] + all_in.plength[10];
	for(int i=0; i<all_in.plength[11]; i++){
		all_in.inputs_names[p0 + i]="Height_l1";
		if(h1_relax[i] == 1){
			all_in.priors_names[p0 + i]="Jeffreys";
			all_in.priors(0, p0 + i)=1;  // Jeffreys prior, with hmin=1
			all_in.priors(1, p0 + i)=100000;  // Jeffreys prior, with hmax=100000... Valid for Main-Sequence Star. Might be ok for Giants as well
		...
		all_in.inputs[p0 + i]=h1_inputs[i];
		all_in.relax[p0 + i]=h1_relax[i];

	}
	// --- Put the Wl1 ---
	p0=all_in.plength[0] + all_in.plength[1] + all_in.plength[2] + all_in.plength[3] + all_in.plength[4] + all_in.plength[5] + all_in.plength[6] + all_in.plength[7] + all_in.plength[8] + all_in.plength[9] + all_in.plength[10] + all_in.plength[11];
	for(int i=0; i<all_in.plength[12]; i++){
		all_in.inputs_names[p0 + i]="Width_l1";
		if(w1_relax[i] == 1){
			all_in.priors_names[p0 + i]="Jeffreys";
			all_in.priors(0, p0 + i)=0.05;  // Jeffreys prior, with hmin=0.4
			if(inputs_MS_global.Dnu <= 40){ 
				all_in.priors(1, p0 + i)=inputs_MS_global.Dnu/3;  // Jeffreys prior, with hmax=40 or set to Dnu/3 if Dnu is clearly for an evolved star... 
			} else{
				all_in.priors(1, p0 + i)=40.;
			}
		...
		all_in.inputs[p0 + i]=w1_inputs[i];
		all_in.relax[p0 + i]=w1_relax[i];
	}
	
	// --- Put the Sn1m ---
	p0=all_in.plength[0] + all_in.plength[1] + all_in.plength[2] + all_in.plength[3] + all_in.plength[4] + all_in.plength[5] + all_in.plength[6] + all_in.plength[7] + all_in.plength[8] + all_in.plength[9] + all_in.plength[10] + all_in.plength[11] + all_in.plength[12];

	/*std::cout << "p0=" << p0 << std::endl;
	std::cout << "all_in.plength[12]=" << all_in.plength[12] << std::endl;
	std::cout << "Sn1m_in.inputs_names: " << std::endl;
	for(int i=0; i<Sn1m_in.inputs_names.size(); i++){
		std::cout << Sn1m_in.inputs_names[i]   <<  "    " << Sn1m_in.inputs[i]  << Sn1m_in.relax[i] << std::endl;
	}
	std::cout << "----" << std::endl;
	*/
	for(int i=0; i<all_in.plength[13]; i++){
		all_in.inputs_names[p0 + i]=Sn1m_in.inputs_names[i];
		all_in.priors_names[p0 +  i]=Sn1m_in.priors_names[i];
	}
	
	all_in.inputs.segment(p0 , all_in.plength[13])=Sn1m_in.inputs;
	all_in.relax.segment(p0 , all_in.plength[13])=Sn1m_in.relax;
	all_in.priors.block(0, p0, Nmax_prior_params, all_in.plength[13])=Sn1m_in.priors;
	// ----------------------------------------------
	...
	if(verbose == 1){
		...
		if(all_in.extra_priors[0] == 0){ 
			...
			std::cout << "   freq_smoothness is set to 1 ==> APPLIES a smoothness condition on l=0,2,3 frequencies (l=1 EXCLUDED)" << std::endl;
