...
if((bool_a1cosi == 0) && (bool_a1sini == 0)){ // Case where Inclination and Splitting_a1 are supposed to be used 
	if( (all_in.model_fullname == "model_MS_Global_a1etaa3_HarveyLike") || (all_in.model_fullname == "model_MS_Global_a1etaa3_Harvey1985")){
		...
		if( Inc_in.priors_names[0]=="Fix" && Snlm_in.priors_names[0]=="Fix"){
			...
			for(int k=0; k<Nmax_prior_params; k++){ // The range of the new prior is defined by the range given by sqrt(splitting_a1) because cos(i) and sin(i) E [-1,1]
	...
	for(int i=0; i<all_in.plength[7]; i++){
		...
		if(w_relax[i] == 1){
			...
			if(inputs_MS_global.Dnu <= 40){ 
				all_in.priors(1, p0 + i)=inputs_MS_global.Dnu;  // Jeffreys prior, with hmax=25... Valid for Main-Sequence Star. DOES NOT WORK FOR RED GIANTS (Should be smaller than Dnu)
			} else{
